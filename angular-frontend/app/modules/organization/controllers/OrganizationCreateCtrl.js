...
app.controller('OrganizationCreateCtrl', function ($scope, $http, $q, $rootScope, OrganizationService, $translatePartialLoader, NgMap, Notification, trans) {
    ...
    $scope.placeChanged = function () {
        ...
        var building = $scope.place.address_components && $scope.place.address_components.length ? $scope.place.address_components[0].long_name : '';
        $scope.organization.website = $scope.place.website;
        $scope.organization.name = $scope.place.name;
        $scope.organization.city = $scope.place.address_components && $scope.place.address_components.length ? $scope.place.address_components[2].long_name : '';
        ...
        $scope.organization.description = $scope.place.reviews && $scope.place.reviews.length ? $scope.place.reviews[0].text : '';
        $scope.organization.website = $scope.place.website;
        $scope.organization.avatar_url = $scope.place.icon;
        $scope.organization.type = $scope.place.types && $scope.place.types.length ? $scope.place.types[0] : '';
